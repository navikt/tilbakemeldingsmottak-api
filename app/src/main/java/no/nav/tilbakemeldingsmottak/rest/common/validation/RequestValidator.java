...
public abstract class RequestValidator {
    ...
    protected void hasText(String input, String feltnavn, String condition) {
        if (StringUtils.isBlank(input)) {
            throw new ClientErrorException(String.format("%s er påkrevd%s", feltnavn, condition));
        }
    }
    ...
    protected void isNull(Object input, String feltnavn, String condition) {
        if (input != null) {
            throw new ClientErrorException(String.format("%s kan ikke være satt%s", feltnavn, condition));
        }
    }
}
