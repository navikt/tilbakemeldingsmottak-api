...
then
    ...
    export SPRING_DATASOURCE_USERNAME=$(cat /var/run/secrets/nais.io/tilbakemeldingsmottak/username)
fi

if test -f /var/run/secrets/nais.io/tilbakemeldingsmottak/password;
then
    echo "Setting SPRING_DATASOURCE_PASSWORD"
    export SPRING_DATASOURCE_PASSWORD=$(cat /var/run/secrets/nais.io/tilbakemeldingsmottak/password)
fi